...
declare var printURL: string;
declare var parlimenURL: string;
declare var featureLayers: any[];
declare var mapLayers: any[];
declare var cleaningServiceInfo: any[];
declare var collectionServiceInfo: any[];
declare var gassetLayer: any[];
declare var baseMapConfig: any[];
declare var defaultBaseMap: string;
...
export class MapviewComponent implements OnInit {
  ...
  parliamens = mapLayers // MAP: 3, 5, 6
  gassetLayer = gassetLayer
  ...
  paperSize = ['A4 Potrait', 'A4 Landscape'];
  selectedPaper = "A4 Potrait";
  pageInfo = {
    'A4 Potrait': { width: 210, height: 297 },
...
*/
  ...
  initMap () {
    let map = new Map({ basemap: defaultBaseMap })
    ...
    let editorExpand = new Expand({
      expandIconClass: "esri-icon-edit", // "esri-icon-basemap", // see https://developers.arcgis.com/javascript/latest/guide/esri-icon-font/
      expandTooltip: "Editor", // optional, defaults to "Expand" for English locale
      view: view,
      content: editor,
      label: 'Editor'
    });
    ...
    */  // MAP: 3, 5, 6
    ...
    let searchExpand = new Expand({
      expandIconClass: "esri-icon-search", // "esri-icon-basemap", // see https://developers.arcgis.com/javascript/latest/guide/esri-icon-font/
    ...
    let source = baseMapConfig.map(t => Basemap.fromId(t.value))
    // console.error('basemapsource')
    // console.log(source)
  ...
  handleLayer (event: MatSlideToggleChange) {
   // console.log(event.checked)
   // console.log(event.source.name)
    const l = this.gassetLayer.find(i => i.name === event.source.name)
    if (l) {
     // console.log('found')
      if (event.checked) {
        this.map.add(l.layer);
        l.show = true
      } else {
        this.map.remove(l.layer);
        l.show = false
      }
    } else {
   //   console.log('missing?')
  ...
  initGasset () {
    for (var c = 0; c < this.gassetLayer.length; ++c) {
      if (this.gassetLayer[c].type === 'basemap') {
        this.gassetLayer[c].layer = new MapImageLayer({
          url: this.gassetLayer[c].url
        });
      } else {
        this.gassetLayer[c].layer = new FeatureLayer({
          url: this.gassetLayer[c].url,
          outFields: ['*']
        });
      }
  ...
  toPrint () {
    ...
    if (this.map && this.view) {
     // console.log(this.map.basemap.title)
      // console.log(this.basemapGallery)
    //  console.log(this.basemapGallery.activeBasemap.id)
      // console.log(this.map.basemap.baseLayers)
      // console.log(this.map.basemap.referenceLayers)
      let searchLayer = {}
      if (this.loadedLayers.length > 0) {
        searchLayer = {
          definitionExpression: this.loadedLayers[0].definitionExpression,
          url: this.loadedLayers[0].url + '/' + this.loadedLayers[0].layerId,
          symbolURL: this.loadedLayers[0].renderer.symbol.url
        }
      }
      let mapSettings = {
        baseMap: this.basemapGallery.activeBasemap.id,
        parlimenURL: parlimenURL,
        x: this.view.center.longitude,
        y: this.view.center.latitude,
        zoom: this.view.zoom,
        gassetLayer: this.gassetLayer.filter(i => i.show === true),
        searchLayer: searchLayer
      }
      localStorage.setItem('mapSettings', JSON.stringify(mapSettings)) //save for future
      var pageSettings = this.pageInfo[this.selectedPaper]
      pageSettings.margin = 0
      pageSettings.unit = 'mm'
      localStorage.setItem('page', JSON.stringify(pageSettings)) //save for future
      var printWindow = window.open(printURL)

      // mapSetting.basemap = this.map.basemap
      // localStorage.setItem('settings', JSON.stringify(mapSetting))
    }
  }
  ...
  ngOnInit () {
   ...
   // console.log(this.dbkl_access_token);
   var modal = document.getElementById("myModal");
    ...
    this.http
      ...
      .subscribe(
        (res) => {
         ...
         // console.log('Parlimen1: ' + res[0].parlimen)
          this.parlimen = localStorage.getItem("parlimen");
          this.findSelectedParliament()
        //  console.log('Parlimen2: ' + this.parlimen)
    ...
    localStorage.setItem("path", "/dbkl/mapview");
  }


  findSelectedParliament () {
    var that = this
    var sp = mapLayers.find(item => item.label === that.parlimen.toUpperCase())
    if (sp) {
      this.selectedParliament = sp.value
    }
  }
  ...
  //  console.log(featureLayer)
    ...
    const featureTable = new FeatureTable({
      ...
      layer: featureLayer,
      editingEnabled: false, // MAP: 3, 5, 6
      fieldConfigs: [
        {
          name: "OBJECTID",
          label: "ID",
